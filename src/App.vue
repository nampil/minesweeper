...
<script setup>
  ...
  const setInit = () => {
    ...
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < rows; j++) {
        cells[i][j].countBees(cells, cols, rows)
      }
    }
    grid.value = cells
  }
...
<style lang="scss">
  ...
  *::before {
  ...
  .settings-enter-active:not(.bee, .vacia),
  .settings-leave-active:not(.bee, .vacia) {
    transition: all 200ms ease-in-out;
  }
